...
//just made for esely change parameter
#define currentalphabet ALPHABET::SPACEALL
#define algo AtmCongruentialPMS

using namespace std;

int main()
{
	_setmode(_fileno(stdout), _O_U16TEXT); // used unicode char in windows cmd
	...
	//Key
	wstring key = L"Vincent le génie des mathématiques pures";

	//time at programm start used to calculate time elapsed
	DWORD64 time = GetTickCount64();

	//encrypt with algo
	wstring __new = Cryptools::algo(start, key, currentalphabet, MODE::CRYPT);
	
	//disp string and encrypt strign
	wcout << L"Original : \n" << start << L"\n\n\n\n" << L"Chiffre : \n" << __new;
...
//SAMPLE CODE USED TO FIX REPETITION INTO ALPHABETS STRINGS
/*
int main()
{
